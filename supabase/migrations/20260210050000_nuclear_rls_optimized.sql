-- NUCLEAR RLS CLEANUP & OPTIMIZATION
-- This script wipes all existing RLS policies and re-implements them 
-- using the optimized (SELECT auth.uid()) pattern to resolve linter warnings.

-- 1. UTILITY: FIX SECURITY DEFINER VIEWS
DO $$
BEGIN
...
-- 2. ENSURE PROFILES HAS ROLE
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS role text DEFAULT 'user';

-- 3. UTILITY: FUNCTION TO DROP ALL POLICIES ON A TABLE
...
-- 4. ENSURE IS_ADMIN IS OPTIMIZED
...
-- 6. CONTENT POLICIES (Public Read)
DO $$
DECLARE
    t text;
