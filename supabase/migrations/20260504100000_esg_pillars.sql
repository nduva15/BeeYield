...
INSERT INTO public.esg_pillars (title, icon, color, impact, initiatives_json)
SELECT * FROM (VALUES
...
) AS v(title, icon, color, impact, initiatives_json)
WHERE NOT EXISTS (SELECT 1 FROM public.esg_pillars WHERE public.esg_pillars.title = v.title);
