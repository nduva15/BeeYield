...
DROP POLICY IF EXISTS "Allow all access to dataset_imports" ON public.dataset_imports;
...
DROP POLICY IF EXISTS "Allow all access to feeding_schedules" ON public.feeding_schedules;
...
DROP POLICY IF EXISTS "Allow all access to knowledge_facts" ON public.knowledge_facts;
...
DROP POLICY IF EXISTS "Allow all access to yield_projections" ON public.yield_projections;
CREATE POLICY "Allow all access to yield_projections" ON public.yield_projections FOR ALL USING (true) WITH CHECK (true);

-- Apiary sizing runs
CREATE TABLE IF NOT EXISTS public.apiary_sizing_runs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  device_id TEXT NOT NULL,
...
DROP POLICY IF EXISTS "Allow all access to apiary_sizing_runs" ON public.apiary_sizing_runs;
CREATE POLICY "Allow all access to apiary_sizing_runs" ON public.apiary_sizing_runs FOR ALL USING (true) WITH CHECK (true);

-- Seed knowledge facts if not already seeded
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.knowledge_facts WHERE is_default = true) THEN
