...
const Receipt = () => {
        ...
        return (
            <div className="min-h-screen flex flex-col items-center justify-center gap-6 bg-[#F8FAFC]">
                ...
                <div className="text-center">
                    <h1 className="text-3xl font-black text-slate-900 mb-2">Order Not Found</h1>
    ...
    return (
        <div className="min-h-screen bg-[#F1F5F9] py-12 px-4 print:bg-white print:py-0 print:px-0">
            {/* Toolbar */}
            <div className="max-w-4xl mx-auto mb-8 flex items-center justify-between print:hidden">
                <Button variant="ghost" onClick={() => navigate(-1)} className="gap-2 text-slate-600 hover:text-slate-900 group">
            ...
            {/* Receipt Card */}
            <Card className="max-w-4xl mx-auto bg-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] border-none overflow-hidden rounded-[2rem] print:shadow-none print:rounded-none">
                {/* Status Bar */}
                <div className="bg-primary/10 py-3 px-8 flex justify-between items-center print:hidden">
                    ...
                    <div className="text-slate-500 text-xs font-medium">
                        Payment via <span className="text-slate-900 uppercase">{order.payment_method}</span>
                    </div>
                </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                                ...
                                <div className="space-y-2">
                                    ...
                                    {order.shipping_address ? (
                                        <div className="text-slate-900 font-bold space-y-1">
                        ...
                        <div className="md:text-right flex flex-col justify-between items-start md:items-end">
                            <div>
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-lg font-black text-slate-900">#{order.order_number}</p>
                                </div>
                            </div>
                            <div className="mt-6 md:mt-0">
                                <p className="text-sm text-slate-500">Date Issued</p>
                                <p className="text-lg font-black text-slate-900 uppercase">{new Date(order.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-10">
                                        ...
                                        <tr key={idx} className="group transition-colors border-b border-slate-50 last:border-0">
                                            <td className="py-8 pr-4">
                                                <div className="flex items-center gap-6">
                                                    <div className="w-20 h-20 bg-slate-100 rounded-2xl overflow-hidden flex-shrink-0 relative group-hover:scale-110 transition-transform duration-500">
                                                        {item.product_image ? (
                                            ...
                                            <td className="py-8 px-4 text-center font-black text-slate-900 text-xl">{item.quantity}</td>
                                            <td className="py-8 px-4 text-right font-bold text-slate-500">{formatCurrency(item.unit_price)}</td>
                                            <td className="py-8 pl-4 text-right font-black text-slate-900 text-xl">{formatCurrency(item.total_price)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        <div className="mt-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-10">
                            ...
                            <div className="w-full md:w-80 space-y-4">
                                ...
                                <div className="pt-6 border-t font-black text-slate-900">
