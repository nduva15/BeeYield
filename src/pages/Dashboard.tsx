        ...
        <div className="min-h-screen bg-neutral-50 p-8 space-y-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-neutral-500 mt-1">Real-time insights into your platform's performance.</p>
                </div>
                    ...
                    <CardContent className="h-[400px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={pageViews}>
                                ...
                                />
                                <Legend />
                                <Area type="monotone" dataKey="views" stroke="#f59e0b" fillOpacity={1} fill="url(#colorViews)" name="Page Views" strokeWidth={2} />
                                <Area type="monotone" dataKey="visitors" stroke="#6366f1" fillOpacity={1} fill="url(#colorVisitors)" name="Unique Visitors" strokeWidth={2} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </CardContent>
                </Card>
                    ...
                    <CardContent>
                        <div className="space-y-4">
                            {topPages.map((page, index) => (
                                <div key={index} className="flex items-center justify-between p-3 bg-white rounded-lg border border-neutral-100 hover:border-neutral-200 transition-colors">
                                    <span className="text-sm font-medium text-neutral-700 truncate max-w-[200px]">{page.page_path}</span>
                                    <div className="flex items-center gap-3">
                                        <span className="text-sm font-semibold text-neutral-900">{page.views.toLocaleString()} views</span>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={scans}>
                                ...
                                <Tooltip
                                    ...
                                    labelFormatter={(label) => new Date(label).toLocaleDateString()}
                                />
                                <Bar dataKey="scans" fill="#f59e0b" radius={[4, 4, 0, 0]} name="QR Scans" />
                            </BarChart>
                        </ResponsiveContainer>
                    </CardContent>
                </Card>
...
const SummaryCard = ({ title, value, icon, trend }: { title: string, value: string, icon: React.ReactNode, trend: string }) => (
    <Card className="shadow-sm border-neutral-200 hover:shadow-md transition-shadow">
        <CardContent className="p-6">
            ...
            <div className="flex items-center mt-4 text-xs font-medium text-emerald-600 bg-emerald-50 w-fit px-2 py-1 rounded-full">
