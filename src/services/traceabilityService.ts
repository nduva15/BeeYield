...
export const traceBatch = async (code: string): Promise<TraceResponse | null> => {
    ...
    try {
        ...
        if (error?.status === 404) {
            ...
            throw new Error(`Batch ${normalizedCode} not found. Please check the code and try again.`);
        }
        ...
        if (error.message && (
            ...
            throw new Error(`Connection Error: Unable to reach the BeeYield server. Please try again later.`);
        }
