...
export default function InspectionsPage({ isOpen = true, onClose, embedded = false }: { isOpen?: boolean; onClose?: () => void; embedded?: boolean }) {
  ...
  const allApiaries = useMemo(() => {
    if (userApiaries.length > 0) {
      return deduplicateApiaries(userApiaries.map((ua) => ({ id: ua.id, name: normalizeApiaryName(ua.name), region: "Apiary Zone", county: "Operational" })));
    }
    return [];
  }, [userApiaries]);
  ...
  const allHives = useMemo(() => {
    return userHives.map((uh) => ({
      id: uh.id,
      hive_code: uh.hive_code || uh.name || "HIVE",
      name: uh.name || uh.hive_code || "Hive",
      apiary_name: uh.apiary_name || "",
      frame_count: (uh as any).frame_count || 10,
    }));
  }, [userHives]);
