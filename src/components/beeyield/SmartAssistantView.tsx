...
const SmartAssistantView: React.FC<AIAssistantViewProps> = ({ onTabChange, initialMessage, onInitialMessageConsumed }) => {
    ...
    return (
        <div className="flex flex-col h-full w-full bg-background honeycomb-bg overflow-hidden relative" style={{ minHeight: 'calc(100vh - 60px)' }}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-2xl">
                            {SUGGESTIONS.map((s) => (
                                <button
                                    key={s}
                                    onClick={() => send(s)}
                                    className="text-left px-3 py-2.5 rounded-lg text-xs border border-border hover:border-primary/50 hover:bg-muted transition-all text-muted-foreground hover:text-foreground leading-relaxed shadow-sm"
                                >
                                    {s}
                                </button>
                            ))}
            ...
            {/* Input area */}
            <div className="flex-shrink-0 border-t border-border bg-sidebar px-4 pb-4 pt-3 mt-auto">
                {messages.length > 0 && (
                ...
                <form onSubmit={handleSubmit} className="flex gap-2 items-end max-w-4xl mx-auto">
                    ...
                    {voiceSupported && (
                        <button
                            ...
                            className={`flex-shrink-0 w-11 h-11 rounded-xl border flex items-center justify-center transition-all ${isListening
                    ...
                    <button
                        ...
                        className="flex-shrink-0 w-11 h-11 rounded-xl bg-orange-500 text-white flex items-center justify-center hover:opacity-90 disabled:opacity-40 transition-all shadow-sm"
                    >
