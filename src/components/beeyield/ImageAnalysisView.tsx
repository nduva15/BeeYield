        ...
        <BeeYieldPageShell>
            ...
            <BeeYieldCard className="mb-8">
                <div className="flex items-center gap-3 mb-6 border-b border-border/ pb-4">
                    <div className="w-8 h-8 bg-[#F4D03F]/10 rounded-lg flex items-center justify-center border border-border/">
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-x-12 gap-y-6 pt-2">
                        {instructions.map((item, idx) => (
                            <div key={idx} className="flex flex-col gap-1.5 border-l-2 border-border/ pl-4 py-1">
                                <h4 className="text-sm font-semibold text-foreground">{item.label}</h4>
                                <p className={cn(glass.microLabel, "text-muted-foreground/70")}>{item.description}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {!previewUrl && (
                <div
                    ...
                    className={cn(glass.card, "w-full min-h-[260px] border-dashed border-2 flex flex-col items-center justify-center gap-4 cursor-pointer hover:border-border/ transition-all bg-muted/ shadow-inner p-5")}
                >
                    <input
                        id="bee-image-upload"
                    ...
                    />
                    <div className="w-16 h-16 rounded-2xl bg-[#F4D03F]/5 flex items-center justify-center border border-border/ group-hover:scale-110 transition-transform">
                        <Camera className="w-8 h-8 text-[#F4D03F]/60" />
                    </div>
                    <div className="text-center space-y-2">
                        <h3 className="text-sm font-semibold text-foreground">Choose a photo</h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-500">
                    <div className="lg:col-span-6 space-y-6">
                        <BeeYieldCard className="p-4 relative">
                            <div className="min-h-[300px] flex items-center justify-center bg-muted/ rounded-xl overflow-hidden border border-border/">
                                <img src={previewUrl} alt="Analyzed" className="w-full h-full object-contain max-h-[500px]" />
                                {isAnalyzing && (
                                    <div className="absolute inset-0 bg-muted/ backdrop-blur-sm flex flex-col items-center justify-center z-10">
                                        {realtimeCount > 0 ? (
                                            <>
                            ...
                            <BeeYieldCard>
                                <div className="space-y-6">
                                    <div className="flex items-center gap-3 pb-4 border-b border-border/">
                                        <div className="w-8 h-8 bg-[#F4D03F]/10 rounded-lg flex items-center justify-center border border-border/"><Activity className="w-4 h-4 text-[#F4D03F]" /></div>
                                    ...
                                    <div className="flex items-center justify-between gap-6">
                                        ...
                                        <span className="text-[10px] font-black text-foreground">{results.overallConfidence}%</span>
                                    </div>
                                </div>
                            </BeeYieldCard>
                        )}
                    ...
                    <div className="lg:col-span-6 space-y-6">
                        {isAnalyzing ? (
                            <BeeYieldCard className="h-full flex flex-col justify-center items-center text-center space-y-6 min-h-[400px]">
                                <div className="w-16 h-16 rounded-2xl bg-[#F4D03F]/10 flex items-center justify-center border border-border/">
                            ...
                            <BeeYieldCard className="space-y-6">
                                <div className="flex items-center justify-between border-b border-border/ pb-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="w-8 h-8 rounded-lg bg-[#F4D03F]/10 text-[#F4D03F] flex items-center justify-center font-bold text-xs border border-border/">
                                            {results.beesCounted}
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-6">
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className="text-[10px] font-black text-foreground tabular-nums">{confidenceThreshold}%</span>
                                            </div>
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className="text-[10px] font-black text-foreground tabular-nums">{overlapThreshold}%</span>
                                            </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Select value={displayMode} onValueChange={setDisplayMode}>
                                                <SelectTrigger className={cn(glass.select, "h-9 border-border/ bg-muted/ text-[10px] font-black")}>
                                ...
                                <div className="pt-4 border-t border-border/">
                                    <div className="rounded-xl border border-border/ overflow-hidden bg-muted/ backdrop-blur-sm shadow-sm">
                                        <table className="w-full text-[9px] font-black border-collapse">
                                            ...
                                            <tbody className="divide-y divide-white/20">
                                                {results.detections.slice(0, 6).map((det: DetectionRecord, idx: number) => (
                                                    <tr key={det.id} className="hover:bg-muted/ transition-colors text-foreground">
                                                        ...
                                                        <td className="px-3 py-2.5">
                                                            <span className={cn(
                                                                "px-1.5 py-0.5 rounded bg-[#F4D03F]/10 text-[#F4D03F] border border-border/",
                                                                det.health && det.health.toLowerCase() !== 'healthy' && "bg-red-500/10 text-red-500 border-red-500/10"
                                                            )}>
                                                                {det.health || det.label || 'Unknown'}
                                                            </span>
                                                        </td>
                                                        <td className="px-3 py-2.5 text-muted-foreground/70 font-mono">[{det.bbox?.x ?? 0},{det.bbox?.y ?? 0}]</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                            ...
                            <div className="space-y-6">
                                <BeeYieldCard className="p-8 flex flex-col justify-center items-center text-center space-y-6 md:min-h-[250px]">
                                    <div className="w-12 h-12 rounded-2xl bg-[#F4D03F]/10 flex items-center justify-center border border-border/">
                                    ...
                                    <BeeYieldCard>
                                        ...
                                        <div className="space-y-3">
                                            {recentDetections.slice(0, 3).map((item, i) => (
                                                <div key={i} className="flex items-center justify-between p-3 rounded-xl border border-border/ bg-muted/ hover:bg-muted/ transition-colors">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <p className="text-sm font-semibold text-foreground">Entry {i + 1}</p>
