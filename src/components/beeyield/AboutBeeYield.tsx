...
const StatCard = ({ icon: Icon, value, label }: { icon: any, value: string, label: string }) => (
    <div className="bg-card p-4 rounded-2xl border border-border/ shadow-sm flex flex-col items-center text-center group hover:border-border/ transition-all duration-300">
        ...
        <div className="text-xl font-black text-foreground tracking-tight mb-0.5">{value}</div>
        <div className="text-[8px] font-bold text-foreground/40 tracking-wider">{label}</div>
    </div>
);
...
const CapabilityCard = ({ icon: Icon, title, description }: { icon: any, title: string, description: string }) => (
    <div className="flex gap-4 p-4 rounded-xl hover:bg-[#F4D03F]/5 transition-all group">
        <div className="w-10 h-10 rounded-xl bg-card flex items-center justify-center shadow-sm border border-border/ group-hover:border-border/ flex-shrink-0">
            <Icon className="w-5 h-5 text-[#F4D03F]" />
        </div>
        <div className="flex flex-col gap-0.5">
            <h4 className="text-[11px] font-black tracking-wider text-foreground">{title}</h4>
            <p className="text-[11px] text-foreground/60 leading-relaxed font-semibold">{description}</p>
        </div>
    </div>
);
...
export const AboutBeeYield: React.FC<AboutBeeYieldProps> = ({ onClose }) => {
    return (
        <motion.div 
            ...
            className="w-full max-w-3xl max-h-[85vh] overflow-y-auto custom-scroll bg-card rounded-[2rem] shadow-2xl border border-border/ relative p-6 sm:p-10"
        >
            <button 
                onClick={onClose}
                className="absolute top-6 right-6 w-8 h-8 rounded-full hover:bg-[#F4D03F]/10 flex items-center justify-center transition-all text-foreground/40 hover:text-foreground z-10"
            >
                <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex flex-col gap-10">
                ...
                <div className="flex flex-col items-start gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-2xl font-black text-foreground tracking-tighter leading-none">Our Story</h2>
                    </div>
                ...
                <div className="space-y-6">
                    <div className="flex items-center gap-4">
                        ...
                        <h3 className="text-[9px] font-black text-foreground/30 italic">Research Core</h3>
                        <div className="h-px flex-1 bg-[#F4D03F]/10" />
                    </div>
                ...
                <div className="space-y-6">
                    <div className="flex items-center gap-4">
                        <div className="h-px flex-1 bg-[#F4D03F]/10" />
                        <h3 className="text-[9px] font-black text-foreground/30 italic">Capabilities</h3>
                        <div className="h-px flex-1 bg-[#F4D03F]/10" />
                    </div>
