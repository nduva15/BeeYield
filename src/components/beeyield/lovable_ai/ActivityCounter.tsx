    ...
    intervalRef.current = window.setInterval(() => {
      setSeconds((s) => {
        if (s <= 1) {
          ...
          toast.success(`Counter complete: ${count} bees/min`);
          return 0;
        }
        return s - 1;
      });
  ...
  const band =
    count < 20
      ? {
          ...
          note: "Investigate queen status, weather, dearth.",
        }
      : count < 60
        ? {
            label: "Normal early/late season",
            colour: "text-foreground",
            note: "Healthy baseline activity.",
          }
        : count < 120
          ? { label: "Healthy mid-season", colour: "text-honey", note: "Good nectar flow." }
          : count < 250
            ? {
                label: "Strong nectar flow",
                colour: "text-honey font-bold",
                note: "Confirm super capacity.",
              }
            : {
                label: "Peak / robbing risk",
                colour: "text-destructive font-bold",
                note: "Inspect for swarm prep or robbing.",
              };

  if (!isOpen && !embedded) return null;
  return (
    <div className={embedded ? "w-full flex justify-center py-6" : "fixed inset-0 z-50 bg-background/95 backdrop-blur-sm flex items-center justify-center p-4"}>
      <div className={embedded ? "bg-card border border-border rounded-2xl max-w-xl w-full p-6 shadow-md" : "bg-card border border-border rounded-2xl max-w-md w-full p-6 shadow-2xl"}>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <input
            value={florage}
            onChange={(e) => setFlorage(e.target.value)}
            className="bg-background border border-border rounded-lg px-3 py-2 text-sm"
            placeholder="Florage (e.g. clover)"
          />
        </div>
        ...
        <div className="text-center py-6 mb-4 rounded-xl bg-muted/40 border border-border">
          ...
          <div className="text-xs text-muted-foreground mt-3">
            Time left: <b className="text-foreground">{seconds}s</b>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <button
            ...
            className="px-3 py-2 rounded-lg border border-honey/40 text-honey text-xs flex items-center justify-center gap-1"
          >
            <Play className="w-3 h-3" /> Start
          </button>
          <button
            onClick={() => setRunning(false)}
            className="px-3 py-2 rounded-lg border border-border text-xs flex items-center justify-center gap-1"
          >
        ...
        {count > 0 && (
          <div className="p-3 rounded-lg border border-honey/30 bg-honey/5 mb-3 text-xs">
            <b className={band.colour}>{band.label}</b>
            <br />
        ...
        <button
          onClick={save}
          disabled={count === 0}
          className="w-full px-4 py-2.5 rounded-lg bg-gradient-amber text-primary-foreground font-semibold text-sm disabled:opacity-50"
        >
          Save to Bee Flight Logs
        </button>

        <p className="text-[10px] text-muted-foreground mt-3 text-center">
          Linked to: Bee Flight Tracker · MOA Activity panel · Activity Forecaster · Pollination
          Planning
        </p>
      </div>
    </div>
