...
export default function PollinationCharts({ isOpen, onClose, embedded = false }: PollinationChartsProps) {
  ...
  return (
    <div className={embedded ? "w-full" : "fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm"} onClick={embedded ? undefined : onClose}>
      <div className={embedded ? "bg-card border border-border rounded-2xl shadow-sm w-full max-w-full overflow-hidden flex flex-col" : "bg-card border border-border rounded-2xl shadow-xl w-full max-w-5xl max-h-[90vh] overflow-hidden mx-4 flex flex-col"} onClick={(e) => e.stopPropagation()}>
        {/* Header */}
