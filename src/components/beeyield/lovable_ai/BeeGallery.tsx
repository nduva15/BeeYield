...
export default function BeeGallery({ isOpen, onClose, embedded = false }: BeeGalleryProps) {
  ...
  const header = (
    <div className="flex items-center justify-between border-b border-border/60 px-4 sm:px-6 py-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-display text-lg sm:text-xl font-bold text-foreground">Bee Species Gallery</h2>
  ...
  const body = selected ? (
    ...
    <div className="p-4 sm:p-6">
      <button
        ...
        className="text-xs text-primary hover:underline mb-4 flex items-center gap-1 font-medium"
      >
        <ChevronLeft className="w-3.5 h-3.5" /> Back to all species
      </button>
      <div className="flex flex-col sm:flex-row gap-6">
        <div className="flex-shrink-0 w-full sm:w-60 h-60 rounded-xl overflow-hidden border border-border bg-muted/20">
    ...
    <div className="flex flex-col">
      {/* Search & Filter Bar */}
      <div className="px-4 sm:px-6 pt-4 pb-3 border-b border-border/60 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filtered.map(({ bee, i }) => (
              <button
                ...
                className="text-left rounded-xl border border-border hover:border-primary/50 bg-card hover:bg-muted/40 transition-all group overflow-hidden flex flex-col shadow-sm"
              >
                <div className="aspect-square overflow-hidden bg-muted/20 w-full">
                  <img
                    src={bee.image}
                    alt={bee.name}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-2xl shadow-xl w-full max-w-5xl max-h-[90vh] overflow-hidden my-auto flex flex-col"
