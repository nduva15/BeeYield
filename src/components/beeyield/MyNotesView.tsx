            ...
            <BeeYieldPageShell>
                <BeeYieldPageHeader
                    ...
                    actions={
                        <button
                            ...
                            className={cn(glass.btnSecondary, "px-4 h-8 rounded-xl text-[12px] font-semibold flex items-center gap-2 border-border/")}
                        >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 relative z-10">
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        <BeeYieldCard className={cn("p-5 space-y-6 bg-muted/ border-border/ shadow-xl relative overflow-hidden")}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 relative z-10">
                                <div className="space-y-3">
                                    <label htmlFor="note_date" className="text-[12px] font-semibold text-foreground/70 ml-1">Date</label>
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            <button
                                                ...
                                                className={cn(glass.btnSecondary, "w-full justify-between h-10 px-4 rounded-xl border-border/ bg-muted/ hover:bg-muted/ group shadow-sm transition-colors")}
                                            >
                                                <span className="font-semibold text-[12px] text-foreground">{format(noteDate, "dd/MM/yyyy")}</span>
                            ...
                            <div className="space-y-3 relative z-10">
                                <label htmlFor="note_details" className="text-[12px] font-semibold text-foreground/70 ml-1">Details</label>
                                <textarea
                                    id="note_details"
                                    name="note_details"
                                    autoComplete="off"
                                    ...
                                    className={cn(glass.input, "w-full min-h-[260px] p-4 leading-relaxed resize-none bg-muted/ border-border/ hover:bg-muted/ focus:bg-white transition-colors text-[13px] font-medium shadow-sm")}
                        ...
                        <div className="flex items-center gap-4 pt-2">
                            ...
                            <button
                                type="button"
                                onClick={() => { setIsAddingNote(false); setIsEditingNote(null); }}
                                className={cn(glass.btnSecondary, "px-8 h-9 text-[12px] font-semibold rounded-xl border-border/ shadow-sm")}
                            >
                                Cancel
                            </button>
                        </div>
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        <BeeYieldCard className={cn("p-5 space-y-6 bg-muted/ border-border/ shadow-lg")}>
                            {/* Priority Selection */}
                            <div className="space-y-4">
                                <label className="text-[12px] font-semibold text-foreground/70 ml-1">Priority</label>
                                            ...
                                            className={cn(
                                                ...
                                                priority === p
                                                    ? cn(
                                                        ...
                                                        p === 'medium' && "bg-[#F4D03F]/10 text-[#F4D03F] border-border/ shadow-md",
                                                        p === 'high' && "bg-red-500/10 text-red-500 border-red-500/20 shadow-md"
                                                    )
                                                    : "bg-muted/ border-gray-100 text-muted-foreground/70 hover:bg-muted/"
                                            )}
                                        >
                            ...
                            {/* Category Selection */}
                            <div className="space-y-4">
                                <label className="text-[12px] font-semibold text-foreground/70 ml-1">Category</label>
                                    ...
                                    {['General', 'Health', 'Queen Seen', 'Harvest', 'Varroa'].map((cat) => (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                category === cat
                                                    ? "bg-[#F4D03F] text-foreground border-[#F4D03F] shadow-lg shadow-[#F4D03F]/20"
                                                    : "bg-muted/ border-gray-100 text-muted-foreground/70 hover:text-foreground hover:bg-muted/"
                                            )}
                                        >
                                            {cat}
                                        </button>
                            ...
                            {/* Entity Link */}
                            <div className="space-y-4">
                                <label className="text-[12px] font-semibold text-foreground/70 ml-1">Link to location (optional)</label>
                                <div className="space-y-3">
                                    <div className="relative group">
                                        ...
                                        <select
                                            ...
                                            className={cn(glass.select, "w-full pl-10 text-[12px] font-medium bg-muted/ h-10")}
                                        >
    ...
    return (
        <BeeYieldPageShell>
            ...
            <motion.div
                ...
                className={cn(glass.filterBar, "p-1.5 flex items-center gap-2 bg-muted/ backdrop-blur-md rounded-2xl border-border/")}
            >
                <div className="flex flex-col md:flex-row gap-2 items-center w-full">
                    <div className="relative flex-1 group/sel w-full">
                        ...
                        <select
                            ...
                            value={selectedPlaceId || ""}
                            onChange={(e) => setSelectedPlaceId(e.target.value || null)}
                            className={cn(glass.select, "w-full pl-10 h-10 text-[12px] font-medium bg-muted/")}
                    ...
                    <div className="relative flex-1 group/search w-full">
                        ...
                        <input
                            ...
                            className={cn(glass.input, "w-full pl-10 h-10 text-[12px] font-medium bg-muted/ border-border/")}
                        />
                    </div>

                    <div className="flex items-center gap-3 px-4 h-10 bg-muted/ rounded-xl border border-border/ shadow-sm shrink-0">
                        <div className="w-1.5 h-1.5 rounded-full bg-[#1B9157] shadow-sm shadow-[#1B9157]/50 animate-pulse" />
                        <span className="text-[11px] font-semibold text-muted-foreground/90 whitespace-nowrap">
            ...
            <div className="relative z-10 pt-6">
                {filteredNotes.length === 0 ? (
                    <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className={glass.emptyState}>
                        <div className="w-20 h-20 rounded-xl bg-[#F4D03F]/5 border border-border/ flex items-center justify-center mb-6">
                            <StickyNote className="w-8 h-8 text-[#F4D03F] opacity-30" />
                        </div>
                        <div className="space-y-4">
                            <h3 className="text-lg font-bold text-foreground tracking-tight">No notes yet</h3>
                            <p className="text-sm text-muted-foreground max-w-md mx-auto">No notes match these filters.</p>
                        </div>
                        ...
                        <AnimatePresence>
                            {filteredNotes.map((note, i) => (
                                <motion.div
                                    ...
                                    className={cn(glass.card, "p-5 hover:border-border/ group flex flex-col h-full bg-muted/ border-border/ shadow-xl relative overflow-hidden")}
                                >
                                    ...
                                    <div className="flex items-center justify-between mb-5 relative z-10">
                                        <div className={cn("px-3 py-1 text-[11px] font-semibold rounded-lg border", 
                                            ...
                                            note.priority === 'medium' ? "bg-[#F4D03F]/10 text-[#F4D03F] border-border/" : 
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                ...
                                                className="w-9 h-9 rounded-xl bg-muted/ border border-border/ flex items-center justify-center hover:bg-[#F4D03F] hover:text-white transition-all shadow-sm"
                                            ...
                                            <button
                                                ...
                                                className="w-9 h-9 rounded-xl bg-muted/ border border-border/ flex items-center justify-center hover:bg-red-500 hover:text-white transition-all shadow-sm"
                                    ...
                                    <div className="space-y-4 relative z-10 flex-1">
                                        <h4 className="text-[14px] font-bold text-foreground tracking-tight line-clamp-2 group-hover:text-[#F4D03F] transition-colors leading-snug">
                                            {note.title?.trim() || "Untitled note"}
                                        </h4>

                                        <div className="bg-muted/ rounded-2xl p-5 border border-border/ flex-1 min-h-[100px] shadow-inner relative overflow-hidden">
                                            <div className="absolute top-2 right-2 opacity-10"><StickyNote className="w-8 h-8" /></div>
                                            <p className="text-[12px] font-medium text-muted-foreground/90 leading-relaxed line-clamp-4 relative z-10">
                                                {note.content?.trim() || "No details yet."}
                                            </p>
                                        </div>
                                    </div>

                                    <div className="pt-5 border-t border-border/ flex flex-wrap gap-4 mt-6 relative z-10">
                                        <div className="flex items-center gap-2">
                                            <div className="w-8 h-8 rounded-lg bg-muted/ border border-border/ flex items-center justify-center shadow-sm">
                                                <ClockIcon className="w-3.5 h-3.5 text-[#F4D03F]" />
                                            </div>
                                            <span className="text-[12px] font-medium text-muted-foreground/90 tabular-nums">{format(new Date(note.note_date || note.created_at!), "dd MMM yyyy")}</span>
                                        </div>
                                        {note.hive_id && (
                                            <div className="flex items-center gap-2 ml-auto">
                                                <span className="text-[12px] font-semibold text-white bg-[#1B9157] px-3 py-1.5 rounded-xl border border-border/ shadow-sm transition-colors">
