...
import {
    ...
    AlertCircle, AlertTriangle, History, Send
} from 'lucide-react';
...
const requestReference = (request: SupportRequest) =>
    request.reference_id || `REQ-${request.id.slice(0, 8).toUpperCase()}`;
...
export const SupportRequestsTab: React.FC = () => {
    ...
    const filteredRequests = (requests || []).filter(req => {
        ...
        const matchesSearch =
            ...
            requestReference(req).toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const getStatusIcon = (status: string) => {
        switch (normalizeStatus(status)) {
            ...
            case 'Draft': return <AlertTriangle className="h-4 w-4 text-slate-500" />;
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                    {['all', 'Open', 'In Progress', 'Resolved', 'Draft'].map((status) => (
                        <Button
                            key={status}
                            ...
                            {isLoading ? (
                                ...
                                filteredRequests.map((req) => (
                                    <TableRow key={req.id} className="hover:bg-muted/20 transition-colors border-border/10 cursor-pointer" onClick={() => setSelectedRequest(req)}>
                                        <TableCell className="px-6 font-mono text-xs text-primary">{requestReference(req)}</TableCell>
                                        ...
                                        <TableCell className="px-6">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-medium">{normalizeStatus(req.status)}</span>
                                            </div>
                                        </TableCell>
                        ...
                        <>
                            <DialogHeader className="p-6 border-b border-border/50 bg-muted/30">
                                <div className="flex items-center justify-between mb-2">
                                    <Badge variant="outline" className="font-mono text-[10px]">{requestReference(selectedRequest)}</Badge>
                                            ...
                                            comments?.map((comment) => (
                                                ...
                                                >
                                                    <div className={cn(
                                                        ...
                                                        {comment.message || (comment as any).comment_text}
                                                    </div>
                                                    <div className="flex items-center gap-2 text-[10px] text-muted-foreground px-1">
                                                        <span>{comment.is_internal || comment.author_id !== selectedRequest.user_id ? 'BeeYield Support' : 'Farmer'}</span>
                                                        <span>•</span>
