...
Traceability and BeeYield Ledger (Golden Thread):
...
- Customers can verify honey authenticity by entering their batch code on the Traceability page, which queries the ledger to confirm: "Yes, this matches the records exactly."
- The BeeYield ledger is immutable, meaning records cannot be changed or deleted, building 100 percent trust with customers.
- BeeYield Traceability currently has 366 plus blocks loaded and growing.
- Backend endpoint: /api/v1/traceability/code/{code} returns the complete history for a batch.
...
- Every harvest is automatically sealed on BeeYield ledger with a unique batch code generated by the backend.
...
- Uses BeeYield ledger to create immutable records that cannot be altered or deleted.
