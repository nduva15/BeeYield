...
def safe_insert(table, data):
    try:
        ...
        print(f" [OK] Inserted into {table}")
        return res
...
def cleanup():
    ...
    if os.path.exists(chain_path):
        try:
            ...
            print(f"[ERROR] Could not delete blockchain file.")
...
def seed():
    try:
        ...
        safe_insert("farmers", farmer_data)
        ...
        safe_insert("apiaries", apiary_data)
        ...
        for i in range(1, 185):
            ...
            if i % 50 == 0:
                ...
                safe_insert("hives", h_to_supabase)
                h_to_supabase = []
        ...
        for i in range(1, 16):
            ...
            harv_date_str = (date(2026, 1, 3) + timedelta(days=(i % 7))).isoformat()
            harv_data = {
                "id": harv_uuid,
            ...
            # Remove harvester_name from Supabase insert (not in schema)
            s_harv_data = harv_data.copy()
            if "harvester_name" in s_harv_data:
                del s_harv_data["harvester_name"]
            safe_insert("harvests", s_harv_data)
            
            # 6c. Processing
            proc_uuid = str(uuid.uuid4())
            proc_date_str = date(2026, 1, 12).isoformat()
            proc_data = {
                "id": proc_uuid,
                "harvest_id": harv_uuid,
            ...
            # Remove processing_method from Supabase insert (not in schema)
            s_proc_data = proc_data.copy()
            if "processing_method" in s_proc_data:
                del s_proc_data["processing_method"]
            safe_insert("processing_records", s_proc_data)
            ...
            # Try packaged_batches then batches
            try:
                supabase.table("packaged_batches").insert(s_batch).execute()
                print(f" [OK] Inserted into packaged_batches: {batch_code}")
            except:
                safe_insert("batches", s_batch)
            ...
            safe_insert("honey_batches", h_batch)
