...
def seed():
    try:
        ...
        for i in range(1, 16):
            ...
            s_harv = harv_data.copy()
            s_harv["harvest_date"] = harv_date_obj.isoformat()
            supabase.table("harvests").insert(s_harv).execute()
            ...
            # Prepare for supabase
            s_proc = proc_data.copy()
            s_proc["processing_date"] = proc_date_obj.isoformat()
            supabase.table("processing_records").insert(s_proc).execute()
        ...
        with open("backend/error_trace.txt", "w") as f:
