...
def cleanup():
    ...
    if os.path.exists(chain_path):
        ...
        print(f"✓ Deleted blockchain file at {chain_path}")
    ...
    for table in tables:
        try:
            ...
            print(f"✓ Cleared Supabase table: {table}")
        except Exception as e:
            print(f"⚠️ Could not clear {table}: {e}")

def seed():
    print("\n--- 2. Initializing New HoneyChain ---")
    # We pass difficulty=1 for speed during seeding
    ...
    print(f"✓ Farmer registered: {farmer_data['name']}")
    ...
    print(f"✓ Apiary registered: {apiary_data['name']}")

    print("\n--- 5. Seeding 184 Hives ---")
    hives_map = {} # Map code to UUID
    h_to_supabase = []
    ...
    if h_to_supabase:
        supabase.table("hives").insert(h_to_supabase).execute()
    print("✓ 184 Hives registered in DB and Blockchain.")

    print("\n--- 6. Seeding Granular Batches for 15 Hives ---")
    # We will create 15 batches, one for each harvested hive KIB-H001 through KIB-H015
    for i in range(1, 16):
        h_code = f"KIB-H{i:03d}"
        h_uuid = hives_map[h_code]
        print(f"Processing Batch for Hive {h_code}...")
        
        # 6a. Record Sensor Data (IoT metrics)
        sensor_data = {
            "hive_id": h_uuid,
            "temperature_celsius": 34.0 + random.uniform(0.1, 0.8),
            "humidity_percent": 50.0 + random.uniform(1.0, 5.0),
            "weight_kg": 40.0 + random.uniform(2.0, 5.0),
        ...
        harv_date = date(2026, 1, 3 + (i % 7)) # Spread over Jan 3 - Jan 9
        harv_data = {
            "id": harv_uuid,
        ...
        harv_data["harvest_date"] = str(harv_date) # convert for supabase
        supabase.table("harvests").insert(harv_data).execute()
        
        # 6c. Record Processing
        ...
        batch_code = f"KIB-{h_code}-0126" # Unique batch code per hive
        batch_data = {
            "id": batch_id,
            "batch_id": batch_id,
            ...
            "jar_count": 8,
            "jar_size_ml": 500,
            "production_date": "2026-01-15T12:00:00",
            "best_before": "2028-01-15",
        ...
        # Supabase Honey Batches (for Shop/Traceability compatibility)
        h_batch = {
            "batch_code": batch_code,
            "honey_type": "Kibwezi Single-Hive Raw",
            "harvest_date": str(harv_date),
    ...
    print("\n✓ SUCCESS: 15 Granular Batches created and linked to specific hives.")
    print("✓ Each scanned hive now shows its unique metrics, location, and story.")
