...
async def ensure_profile(
    *,
    ...
    # Try richer upsert first; if schema is leaner, fall back to minimal id record.
...
async def relink_beeyield_account(
    *,
    ...
    sensor_rows = await db_select("sensor_readings", filters={"apiary_id": apiary_id}, limit=5000, token=lookup_token)
    disease_rows = await db_select("disease_detections", filters={"apiary_id": apiary_id}, limit=5000, token=lookup_token)
