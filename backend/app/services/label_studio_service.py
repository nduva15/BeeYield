...
def generate_advanced_label_pdf(payload: dict) -> bytes:
    ...
    # QR Code (Traceability) - links to batch certificate
    if payload.get("showQRCode"):
        batch_code = payload.get("batchNumber") or payload.get("harvest_batch_id") or ""
        ...
        if batch_code:
            try:
                ...
                # Use the public Traceability UI route (supports ?code=...)
                trace_url = payload.get("traceUrl") or f"https://beeyield.com/traceability?code={batch_code}"
                qr = qrcode.QRCode(
                    version=None,
    ...
    if payload.get("showFooter"):
        # Brand mark: Generated by BeeYield (vector mark + text)
        year = payload.get("harvestYear") or "2025"
        c.saveState()
        c.setFont("Helvetica-Bold", 4.5)
        c.setFillColor(accent_color)

        # Small hex mark (no external asset dependency)
        try:
            cx = (width / 2) - (20 * mm)
            cy = 4 * mm
            r = 1.6 * mm
        ...
        c.setFillColor(text_color)
        c.drawCentredString(width / 2, 4 * mm, f"Generated by BeeYield • {year}")
        c.restoreState()
