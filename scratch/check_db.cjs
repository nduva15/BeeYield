...
async function check() {
  ...
  if(error) console.error(error);
    ...
    const caps = data.filter(p => p.name.includes('Cap') || p.name.includes('Beanie') || p.name.includes('Tee'));
    console.log('Found in DB:', caps);
    const merch = data.filter(p => p.category.includes('merch') || p.category.includes('brand'));
    console.log('Merch category in DB:', merch);
  }
}
check();
