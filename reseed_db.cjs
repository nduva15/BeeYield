...
async function reseedSupabase() {
  ...
  // Delete all existing honey products (cascade delete variants if FK is set, else delete variants first)
  ...
  for (const item of HONEY_ITEMS) {
    // Generate an actual UUID for product so insert works nicely
