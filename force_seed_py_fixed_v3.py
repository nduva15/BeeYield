...
print("--- STARTING DETERMINISTIC SEEDING (PYTHON) ---")
...
try:
    res = s.table("honey_batches").upsert(demoBatches, on_conflict="batch_code").execute()
    print("SUCCESS: Batches seeded.")
except Exception as e:
    print(f"FAIL Batches: {e}")
...
try:
    # Use insert if upsert with name fails? No, the error was likely JSON related to column mismatch
    res = s.table("farmers").upsert(farmer, on_conflict="name").execute()
    print("SUCCESS: Farmer seeded.")
except Exception as e:
    print(f"FAIL Farmer: {e}")
